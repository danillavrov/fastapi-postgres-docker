...
from models import Author, Book, BookOwner, Category, SessionLocal, User
from schemas import (
    AddAuthor,
    AddBook,
    AddCategory,
...
def get_book_by_name(book_name: str):
    ...
    task = get_book.delay(book_name)
    return {"task_number": task.id, "status": "started"}

@app.get("/tasks/{task_id}")
def get_result(task_id: str):
    task = get_book.AsyncResult(task_id)
    if task:
        a = task.get()
        result_str = a.replace("'", '"')

        result_dict = json.loads(result_str)
        result = {"status": "SUCCESS", "result": result_dict}
